...
# RPC host inside the Docker network (default to dogecoin-node service)
RPC_HOST=${rpchost:-dogecoin-node}
...
# ----------------------------------------------------------------------------
wait_for_rpc() {
  echo "Waiting for Dogecoin node RPC (${RPC_USER}@${RPC_HOST}:${RPC_PORT}) …"
  until dogecoin-cli -regtest -rpcuser="${RPC_USER}" -rpcpassword="${RPC_PASS}" \
                     -rpcconnect="${RPC_HOST}" -rpcport="${RPC_PORT}" getblockchaininfo &> /dev/null; do
    sleep 2
  done
...
init_wallet() {
  if [[ -n "${MINER_ADDRESS}" && -n "${MINER_WIF}" ]]; then
    ...
    ADDRESS=$(dogecoin-cli -regtest -rpcuser="${RPC_USER}" -rpcpassword="${RPC_PASS}" \
                       -rpcconnect="${RPC_HOST}" -rpcport="${RPC_PORT}" getnewaddress)
    WIF=$(dogecoin-cli -regtest -rpcuser="${RPC_USER}" -rpcpassword="${RPC_PASS}" \
                       -rpcconnect="${RPC_HOST}" -rpcport="${RPC_PORT}" dumpprivkey "${ADDRESS}")
  ...
  dogecoin-cli -regtest -rpcuser="${RPC_USER}" -rpcpassword="${RPC_PASS}" -rpcconnect="${RPC_HOST}" -rpcport="${RPC_PORT}" \
